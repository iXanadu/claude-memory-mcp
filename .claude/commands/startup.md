...
1. Search persistent memory for prior context:
   - `memory_search` with `scope=shared` — collective wisdom from all projects
   - `memory_search` with `scope=project` — project-specific WIP and decisions

2. Read `claude/CODEBASE_STATE.md` — current technical state, recent work, and next planned tasks
3. Read `claude/CONTEXT_MEMORY.md` — working context, decisions, and ongoing priorities
4. Read `claude/DEPLOYMENT_STANDARDS.md` — installation and registration workflow
5. Read any files in `claude/specs/` if they exist
